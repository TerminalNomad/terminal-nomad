...
export const LocationMap: React.FC<locationmapprops> = ({ location }) => {
  ...
  const containerStyle = { animationDelay: '0.2s' };
  const iframeStyle = { filter: 'invert(90%) hue-rotate(180deg) brightness(90%) contrast(85%)' };
  ...
  return (
    <div classname="w-full mb-8 animate-fade-in-up" style="{containerStyle}">
      ...
      <div classname="w-full h-80 md:h-96 rounded-xl overflow-hidden border border-white/10 shadow-2xl relative bg-slate-900 group">
         <iframe width="100%" height="100%" src="{mapSrc}" title="My Travels Map" the="" magic:="" invert="" colors="" to="" make="" it="" dark,="" rotate="" hue="" to="" fix="" water="" color="" (blue),="" and="" adjust="" contrast="" style="{iframeStyle}" classname="w-full h-full opacity-80 group-hover:opacity-100 group-hover:filter-none transition-all duration-700"/>
