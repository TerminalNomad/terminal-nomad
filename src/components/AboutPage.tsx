  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-5 py-8 animate-fade-in-up">
      ...
      <div className="relative mb-10 card overflow-hidden p-8 md:p-12 text-center md:text-left">
        <div className="absolute top-0 right-0 p-6 opacity-[0.06]"><Wind size={160} aria-hidden="true" /></div>
        <h1 className="text-4xl md:text-6xl font-black text-white font-mono mb-3 relative z-10">THE <span className="text-brand-accent">NOMAD</span></h1>
        <h2 className="text-lg text-slate-300 font-medium relative z-10">Skydiving Instructor · Content Creator · Adrenaline Architect</h2>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-14">
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="bg-brand-accent/10 border border-brand-accent/20 p-7 rounded-xl">
            <p className="font-mono text-brand-accent italic text-sm md:text-base leading-relaxed">"The edge is not where things end. It's where the real view begins."</p>
          </div>
          <a href="mailto:zack@terminalnomad.com" className="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-full bg-brand-accent hover:bg-white text-brand-dark font-bold font-mono text-sm uppercase tracking-widest transition-all hover:scale-105 active:scale-95 shadow-lg shadow-brand-accent/20">Get In Touch</a>
        </div>
      </div>
    </div>
  );
