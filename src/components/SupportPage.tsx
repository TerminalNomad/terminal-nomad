...
export const SupportPage = ({ links }: SupportPageProps) => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto px-5 py-8 animate-fade-in-up">
      ...
      <div className="text-center mb-12">
        ...
        <h1 className="text-3xl md:text-5xl font-black text-white font-mono mb-4">SUPPORT THE <span className="text-brand-accent">JOURNEY</span></h1>
        <p className="text-slate-400 text-base max-w-md mx-auto leading-relaxed">Creating high-quality content and traveling the world isn't cheap. Your support keeps the dream alive and the parachute packed.</p>
      </div>
      <div className="flex flex-col gap-3 max-w-md mx-auto">
        {supportLinks.map((link) => (<SocialButton key={link.id} item={link} />))}
      </div>
      <p className="mt-12 text-center text-slate-500 text-sm">Thank you for being part of the crew. ✈️</p>
    </div>
  );
};
