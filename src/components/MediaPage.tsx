...
import {
  ...
  Wind, Loader2, CreditCard, Globe, Layers, Check,
} from 'lucide-react';

const API_KEY      = import.meta.env.VITE_GOOGLE_API_KEY as string;
const FOLDER_ID    = import.meta.env.VITE_GOOGLE_FOLDER_ID as string;
// Square checkout links per package
const CHECKOUT_FULL  = 'https://square.link/u/KYcE2YR8'; // $50 — full edited video + pics + raw videos
const CHECKOUT_360   = 'https://square.link/u/O6oRAPyV'; // $35 — 360 upgrade (raw Insta360 files)
const CHECKOUT_COMBO = 'https://square.link/u/vRil0l2r'; // $80 — combo of both
const CHECKOUT_URL   = CHECKOUT_FULL; // default for generic "Buy" buttons
...
export const MediaPage = () => {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-5 py-8 animate-fade-in-up">
      ...
      {/* ── Preview intro ─────────────────────────────────────────────────── */}
      <div className="card p-7 border-brand-accent/20 flex flex-col gap-4 mb-12">
        <div className="flex items-center gap-3">
        ...
        <p className="text-slate-400 text-sm leading-relaxed">
          Search your name below to find your blurred preview videos and photos. Click to play, swipe through your shots, and see every moment of your freefall — then pick the package that's right for you.
        </p>
      ...
      <div className="text-center mb-8">
        <h2 className="text-2xl md:text-3xl font-black text-white font-mono mb-2">
          CHOOSE YOUR <span className="text-brand-accent">PACKAGE</span>
        </h2>
        <p className="text-slate-400 text-sm max-w-lg mx-auto">
          Secure checkout through Square. Standard media delivered to your inbox within 24 hours.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 items-stretch">
        ...
        {/* $50 — Full package */}
        <div className="card p-7 border-brand-accent/25 flex flex-col gap-5">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-brand-accent/15 rounded-xl flex items-center justify-center text-brand-accent flex-shrink-0">
              <Package size={20} />
            </div>
            <h3 className="text-white font-bold font-mono text-base leading-tight">Full Video &amp; Photos</h3>
          </div>
          <div className="flex items-baseline gap-1">
            <span className="text-4xl font-black text-white font-mono">$50</span>
          </div>
          <ul className="text-slate-300 text-sm leading-relaxed space-y-2 flex-1">
            <li className="flex items-start gap-2"><Check size={15} className="text-brand-accent flex-shrink-0 mt-0.5" /> Full edited video</li>
            <li className="flex items-start gap-2"><Check size={15} className="text-brand-accent flex-shrink-0 mt-0.5" /> All your photos</li>
            <li className="flex items-start gap-2"><Check size={15} className="text-brand-accent flex-shrink-0 mt-0.5" /> Raw video files included</li>
          </ul>
          <a
            href={CHECKOUT_FULL}
            target="_blank"
        ...
        {/* $35 — 360 upgrade */}
        <div className="card p-7 border-blue-400/30 flex flex-col gap-5">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-blue-500/15 rounded-xl flex items-center justify-center text-blue-400 flex-shrink-0">
              <Globe size={20} />
            </div>
            <h3 className="text-white font-bold font-mono text-base leading-tight">360 Camera Upgrade</h3>
          </div>
          <div className="flex items-baseline gap-1">
            <span className="text-4xl font-black text-white font-mono">$35</span>
          </div>
          <ul className="text-slate-300 text-sm leading-relaxed space-y-2 flex-1">
            <li className="flex items-start gap-2"><Check size={15} className="text-blue-400 flex-shrink-0 mt-0.5" /> Edited 360 camera video</li>
            <li className="flex items-start gap-2"><Check size={15} className="text-blue-400 flex-shrink-0 mt-0.5" /> Raw Insta360 files included</li>
            <li className="flex items-start gap-2"><Check size={15} className="text-blue-400 flex-shrink-0 mt-0.5" /> Edit it yourself in the free Insta360 app</li>
          </ul>
          <a
            href={CHECKOUT_360}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center justify-center gap-2 px-5 py-3 rounded-xl bg-blue-500 hover:bg-white text-white hover:text-brand-dark font-bold font-mono text-sm uppercase tracking-widest transition-all hover:scale-105 active:scale-95 shadow-lg shadow-blue-500/20 mt-auto"
          >
            <ShoppingCart size={15} />
            Buy — $35
            <ExternalLink size={13} />
        ...
        {/* $80 — Combo */}
        <div className="card p-7 border-brand-accent/40 flex flex-col gap-5 relative overflow-hidden" style={{ background: 'linear-gradient(135deg, rgba(6,182,212,0.08), rgba(59,130,246,0.08))' }}>
          <span className="absolute top-4 right-4 px-2.5 py-0.5 rounded-full bg-brand-accent text-brand-dark text-[10px] font-mono font-black uppercase tracking-wider">Best Value</span>
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-brand-accent/15 rounded-xl flex items-center justify-center text-brand-accent flex-shrink-0">
              <Layers size={20} />
            </div>
            <h3 className="text-white font-bold font-mono text-base leading-tight">Everything Combo</h3>
          </div>
          <div className="flex items-baseline gap-2">
            <span className="text-4xl font-black text-white font-mono">$80</span>
            <span className="text-slate-500 text-sm font-mono line-through">$85</span>
          </div>
          <ul className="text-slate-300 text-sm leading-relaxed space-y-2 flex-1">
            <li className="flex items-start gap-2"><Check size={15} className="text-brand-accent flex-shrink-0 mt-0.5" /> Full edited video, photos &amp; raw files</li>
            <li className="flex items-start gap-2"><Check size={15} className="text-brand-accent flex-shrink-0 mt-0.5" /> Edited 360 video + raw Insta360 files</li>
            <li className="flex items-start gap-2"><Check size={15} className="text-brand-accent flex-shrink-0 mt-0.5" /> The complete package, best price</li>
          </ul>
          <a
            href={CHECKOUT_COMBO}
            target="_blank"
      ...
      {/* ── 360 turnaround note ───────────────────────────────────────────── */}
      <div className="flex items-start gap-3 rounded-2xl border border-blue-400/25 bg-blue-500/5 p-5 mb-14">
        <Clock size={18} className="text-blue-400 flex-shrink-0 mt-0.5" aria-hidden="true" />
        <p className="text-slate-300 text-sm leading-relaxed">
          <span className="text-white font-bold">Heads up on 360 videos:</span> editing the 360 footage takes extra time, so it won't be ready right away. I'll get it to you as soon as I possibly can — thanks for your patience!
        </p>
      </div>
